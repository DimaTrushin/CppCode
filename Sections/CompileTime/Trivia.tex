...
\paragraph{Начиная с C++20}

Из коробки в C++20 все еще нельзя написать так
\begin{cppcode}
...
\end{cppcode}
Но можно сделать промежуточный объект \verb"ConstString", который поможет сохранить синтаксис использования как мы ожидаем.
\begin{cppcode}
template<size_t N>
struct ConstString {
  char data[N]{};
...
Выше мы уже использовали связку \verb"IfElse", чтобы проверить выбирать тип в зависимости от условия.
Это лишь нужно для того, чтобы изменить ужасный и длинный синтаксис из STL.
Давайте я запишу другие связки, которые я бы использовал вместо STL-ных.
Связка \verb"IfElse" приведена повторно для полноты картины этого блока.
\begin{cppcode}
namespace ext {
template<bool cond, class T1, class T2>
using IfElse = typename std::conditional<cond, T1, T2>::type;
...
template<class... Ts>
struct AndT : std::conjunction<Ts...> {};

template<class... Ts>
constexpr bool And = AndT<Ts...>::value;

template<class... Ts>
struct OrT : std::disjunction<Ts...> {};

template<class... Ts>
constexpr bool Or = OrT<Ts...>::value;

template<class T>
struct NotT : std::negation<T> {};

template<class T>
constexpr bool Not = NotT<T>::value;
}
\end{cppcode}
Заметьте, что аргументы для логических операторов -- это классы, а не значения.
Со значениями я и так могу делать все логические связки с помощью конструкций языка:
\begin{cppcode}
constexpr bool b1 = true;
constexpr bool b2 = false;
constexpr bool b3 = b1 && b2;
constexpr bool b4 = b1 || b2;
\end{cppcode}
Связки выше предназначены для операций над типами, которые содержат информацию о значении результата.
В этом случае подразумевается, что результат всегда является \verb"constexpr" статической константой с именем \verb"value".
Например
\begin{cppcode}
template<class T>
struct Condition1T {
  static constexpr bool value = /*some condition on T*/;
};

template<class T>
constexpr bool Condition1 = Condition1T<T>::value;

template<class T>
struct Condition2T {
  static constexpr bool value = /*some condition T*/;
};

template<class T>
constexpr bool Condition2 = Condition2T<T>::value;

template<class T>
using Condition3T = AndT<NotT<Condition1T<T>>, Condition2T<T>>;

template<class T>
constexpr bool Condition3 = Condition3T<T>::value;
\end{cppcode}
Как уже говорилось, такие промежуточные классы нужны для того, чтобы их можно было передавать в качестве аргументов в другие шаблоны.
Можно было бы использовать только переменные \verb"Condition1" и \verb"Condition2" и с помощью них задать переменную \verb"Condition3" так
\begin{cppcode}
template<class T>
constexpr bool Condition3 = !Condition2<T> && Condition2<T>;
\end{cppcode}
Однако, если мы хотим сделать операцию фильтрации по условию, то нам надо условие передавать как аргумент.
В этом случае мы не сможем передать \verb"Condition3", но сможем передать \verb"Condition3T" в фильтрующий шаблон.
В некотором смысле эти вспомогательные классы являются костылями вокруг ограничений языка.

...
public:
  ...
  const T& ref() const;
private:
  std::shared_ptr<T*> host_address_;
};
\end{cppcode}
...
\paragraph{Имплементация методов \texttt{Host}}

Теперь пройдемся по имплементации методов.
Из стандартных методов мы удаляем операции копирования, оставляем дефолтный деструктора, а остальные надо имплементировать.
Вначале нам понадобится вспомогательный метод:
\begin{cppcode}
std::shared_ptr<T*> sharedOnMe() {
  return std::make_shared<T*>(ptr()); } // Downcast
\end{cppcode}
...
Методы доступа к адресу объекта выглядят так:
\begin{cppcode}
  T* ptr() {
...
Теперь дефолтный конструктор
\begin{cppcode}
  Host() : host_address_(sharedOnMe());
\end{cppcode}
...
Теперь методы выдачи ручек имплементируется так
\begin{cppcode}
Handle handle() {
  if (!host_address_)
    host_address_ = sharedOnMe();
  return Handle(host_address_);
}
ConstHandle handle() const {
  assert(host_address_);
...
\end{cppcode}
Обратите внимание, что неконстантный метод \verb"handle" проверяет инициализирован ли \verb"host_address_" и если нет, то инициализируем его своим адресом.
Только после этого можно выдать ручку.
Однако, в случае константной ручки, мы не можем менять состояние полей класса.
А именно, мы не можем изменить \verb"host_address_".
Прежде всего отметим, что не инициализированный объект может возникнуть только после move-а из него.
А потому это проблема возникает, когда мы по константной ссылке обратились к объекту, из которого произошел move.
Логически в этом случае не ясно на сколько вообще хорошо поддерживать операцию выдачи ручки.
В нашей текущей реализации будет возвращаться пустая ручка, которая ни на кого не ссылается.
...
\paragraph{Что делать с деинициализированным \texttt{Host}}

Если очень хочется иметь возможность возвращать ручки из хоста, из которого данные были мувныту куда-то, то тут можно принять несколько решений:
\begin{enumerate}
\item Можно всегда инициализировать \verb"host_address_" во всех конструкторах и операторах присваивания.
Главный недостаток будет в том, что на каждый move будет одна аллокация.
Что не очень хорошо.
\begin{cppcode}
  Host(Host&& other) noexcept 
  : host_address_(std::exchange(other.host_address_, other.sharedOnMe())) {
    *host_address_ = ptr(); // host_address_ is always allocated
  } 
\end{cppcode}
В этом случае нам не надо проверять аллоцирован ли \verb"host_address_", так как инвариант класса -- этот \verb"shared_ptr" всегда аллоцирован.
И аналогично
\begin{cppcode}
Host& operator=(Host&& other) noexcept {
  host_address_ = std::exchange(other.host_address_, other.sharedOnMe());
...
\item Можно поле \verb"host_address_" пометить как \verb"mutable".%
\footnote{Ура, мы придумали полезное(?) применение \texttt{mutable}!}
Это позволит относиться к базовому классу как к кэшу.
Мне не очень импонирует это решение, но видимо это неизбежность.
В этом случае еще понадобится вот такая вспомогательная функция%
\footnote{Ура, мы нашли полезное(?) применение \texttt{const\_cast}-у!}
\begin{cppcode}
std::shared_ptr<T*> sharedOnMe() const {
  return std::make_shared<T*>(static_cast<T*>(ptr())); } // Downcast
\end{cppcode}
и тогда
\begin{cppcode}
template<class T>
...
public:
  ...
  ConstHandle handle() const{
    if (!host_address_)
      host_address_ = sharedOnMe();  // converts const Host* to T*
    return ConstHandle(host_address_);
  }
  ...
private:
  mutable std::shared_ptr<T*> host_addres_;
};
\end{cppcode}
Вспомогательная функция нужна, чтобы сделать опасную операцию конвертации константного адреса хоста, на не константный адрес производного класса \verb"T".
Обратите внимание на имплементацию метода \verb"sharedOnMe".
...
\item Уловка с \verb"unique_ptr" для кэша (смотри раздел~\ref{section::fragile} пункт~\ref{section::fragile::nocopymove}) тут не сработает, потому что \verb"unique_ptr" надо так же аллоцировать при каждом муве, что делает это бесполезным.
По сути вся проблема возникает в том, что делать с объектом, из которого сделан move.
\end{enumerate}
...
\begin{cppcode}
operator bool() const {
  return !host_address_.expired() && ptr() != nullptr;
}
bool operator==(nullptr_t) const {
  return !(*this);
}
...
\begin{cppcode}
ConstHandle(const Handle& other) : ConstHandle(other.host_address_) {
}
ConstHandle(Handle&& other) noexcept : ConstHandle(std::move(other.host_address_)) {
}
\end{cppcode}
Эти конструкции работают потому что и \verb"Handle" и \verb"ConstHandle" хранят внутри себя адрес в виде \verb"T*".
Для того, чтобы \verb"ConstHandle" мог получить доступ к приватному полю \verb"host_address_" внутри \verb"Handle", нужно так же объявить другом класс \verb"ConstHandle" внутри класса \verb"Handle".

...
\begin{cppcode}
bool stack_state(bool condition, const std::stacktrace& backtrace = std::stacktrace::current()) {
  if (!condition)
    std::cerr << "Call Stack:\n" << backtrace << '\n';
  return condition;
}
...
\end{cppcode}
Так как вызов функции \verb"stack_state" происходит внутри \verb"assert", то при отключении \verb"assert"-ов, эта функция не будет вызываться.

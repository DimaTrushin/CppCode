...
\item \verb"std::variant" -- это не совсем полноценный стирающий тип.
...
\paragraph{Об имплементации}

В этом разделе лишь кратко обсужу, как можно имплементировать стирающие типы в общем виде на основе идей имплементации стирающих указателей.
Напомню, что пользователь должен предоставить нам следующие данные
\begin{cppcode}
struct FG {
  template<class B>
  struct Interface : B {
...
q.g();
\end{cppcode}
Теперь можно пойти тем же путем, что и в разделе~\ref{}.
Но теперь нам надо внутри \verb"Any" хранить не указатель на объект, а сам объект.
Принципиальная разница в том, что для произвольного объекта уже не годятся операции копирования, перемещения и удаления по умолчанию.
А потому нужно дополнительно сохранить все копирующие и мувающие операции, так же надо сохранить деструктор.
Так же типы могут как иметь дефолтный конструктор и так и не иметь его вовсе, а потому для каких-то типов мы используем дефолтный конструктор для \verb"Any", а для каких-то нет.
Так же могут быть only movable типы, для которых нет копирующих операций.
Добавление этих методов можно сделать двумя разными способами:
\begin{enumerate}
\item Инкапсюлировать обеспечивающие семантику операции в отдельный объект, а именно хранить объект в \verb"std::any" так:
\begin{cppcode}
template<class Data>
class AnyBase {
  ...
...
protected:
  std::any object_;
  const TableType* vtable_;
};
\end{cppcode}
В этом случае нам не надо думать про то как копировать или перемещать \verb"object_" за это отвечает \verb"std::any" или любой другой тип, который мы захотим использовать.

\item Напрямую менеджерить ресурсы объекта.
\begin{cppcode}
template<class Data>
class AnyBase {
  ...
...
\end{cppcode}
В этом случае мы владеем данными по \verb"ptr_".
Но тогда, мы должны в шаблон \verb"Members" добавить необходимые копирующие и перемещающие конструкторы, копирующие и перемещающие операторы, деструктор, возможно дефолтный конструктор.
Тут важно сказать, что к конструкторам нельзя обратиться по адресу.
Тут надо будет налету создавать нужные обертки вокруг соответствующих конструкторов.
\end{enumerate}
Разница между этими способами следующая.
В первом случае мы снимаем с себя ответственность в менеджменте ресурсов самого объекта.
Однако, внутри себя \verb"std::any" будет хранить указатель на витруальную таблицу, где лежат копирующие, перемещающие операции и деструктор.
А это значит, что размер первого решения -- $3$ указателя, а размер второго решения -- $2$ указателя.
Там где размер типа должен быть не больше $2$ указателей приходится использовать второе решение.
С другой стороны первый способ куда быстрее и легче имплементировать.

\paragraph{Связь с \texttt{AnyPtr}}

Так же хорошо бы иметь поддержку для конвертации \verb"Any<Data>" к \verb"AnyPtr<Data>" напрямую, то есть хранить указатель и виртуальную таблицу не на \verb"Any<Data>", а на \verb"object_" и \verb"vtable_" внутри \verb"Any<Data>".
Это избавит от лишних прыжков по памяти.

\paragraph{SOO -- small object optimization}

Для маленьких объектов занимающих, скажем $3-4$ указателя по размеру, можно обеспечить их хранение на локально на стеке, а не выделять память на куче (как по умолчанию делается в \verb"std::any").
Вопросы того, что такое SOO и как его имплементировать затрагиваются в разделе~\ref{section::SharedPtrImplFeatures}.
Более детально стоит посмотреть раздел по работе с сырой памятью~\ref{}.
